...
module.exports = {
	cardConverter: function(cardObject, player, hand, phase, winner, looser) {
		if (winner == 0) {
			...
			if ((player == winner && hand == 1)) {
				var message = {5:0};
			} else if ((player == winner && hand == 2)) {
				var message = {5:1};
			} else if ((player == looser && hand == 1)) {
				var message = {5:2};
			} else if ((player == looser && hand == 2)) {
				var message = {5:3};
			}
			return message;
		}	
	...
	convertToPoker: function(handsObj) {
		var pokerObj = [];
		for (var i = 0, len = handsObj.length; i < len; i++) {
			if (handsObj[i].numbers == 10) {
				handsObj[i].numbers = 'T';
			}
			pokerStr = handsObj[i].numbers[0] + handsObj[i].suits[0];
			pokerObj.push(pokerStr);	
		}
		return pokerObj;
	}
}
